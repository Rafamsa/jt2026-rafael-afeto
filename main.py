...
def main():
    ...
    # 2. FATURAMENTO POR PERFIL
    print("\n[2/5] Calculando faturamento por perfil de imóvel...")
    merged_profile = details_df[['airbnb_listing_id', 'number_of_bedrooms', 'number_of_bathrooms', 'listing_type']].merge(
        revenue_df, on='airbnb_listing_id', how='inner'
    )
    grouped_profile = merged_profile.groupby(['listing_type', 'number_of_bedrooms', 'number_of_bathrooms']).agg(
        faturamento_total=('total_revenue', 'sum'),
        receita_media_anual=('total_revenue', 'mean'),
        tamanho_da_amostra=('airbnb_listing_id', 'count')
    ).reset_index()
    
    filtered_profile = grouped_profile[grouped_profile['tamanho_da_amostra'] > 10].copy()
    sorted_profile = filtered_profile.sort_values(by='receita_media_anual', ascending=False)
    
    print("      -> Gerando gráficos de perfis...")
    sns.set_theme(style="whitegrid")
    sorted_profile['perfil'] = sorted_profile['listing_type'].str.capitalize() + ' | ' + \
                               sorted_profile['number_of_bedrooms'].astype(str) + 'Q | ' + \
                               sorted_profile['number_of_bathrooms'].astype(str) + 'B'
                               
    plt.figure(figsize=(10, 6))
    sns.barplot(data=sorted_profile, x='receita_media_anual', y='perfil', hue='perfil', legend=False, palette='viridis')
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(data=sorted_profile, x='tamanho_da_amostra', y='perfil', hue='perfil', legend=False, palette='magma')
    ...
    merged_bairros = mesh_df[['airbnb_listing_id', 'suburb', 'latitude', 'longitude']].merge(
        revenue_df, on='airbnb_listing_id', how='inner'
    )
    suburb_group = merged_bairros.groupby('suburb').agg(
        faturamento_total=('total_revenue', 'sum')
    ...
    sns.scatterplot(
        data=merged_bairros, x='longitude', y='latitude', hue='suburb',
        size='total_revenue', sizes=(20, 800), alpha=0.6, palette='Set1'
    )
    ...
    # 4. CORRELAÇÃO DE COMODIDADES
    print("\n[4/5] Processando comodidades e correlação (Pearson)...")
    merged_corr = details_df[['airbnb_listing_id', 'owner_id', 'amenities', 'star_rating', 'number_of_reviews']].merge(
        revenue_df, on='airbnb_listing_id', how='inner'
    ).merge(hosts_df[['owner_id', 'is_superhost']], on='owner_id', how='left')
    
    merged_corr['is_superhost'] = merged_corr['is_superhost'].apply(lambda x: 1 if str(x).lower() == 'true' else 0)
    def safe_eval(val):
    ...
    for am in top_10:
        merged_corr[f"am:{am}"] = merged_corr['amenities_list'].apply(lambda x: 1 if am in x else 0)
        
    features = ['star_rating', 'number_of_reviews', 'is_superhost'] + [f"am:{am}" for am in top_10]
    correlations = {}
    for f in features:
    ...
    airbnb_roi = details_df[['airbnb_listing_id', 'number_of_bedrooms']].merge(
        mesh_df[['airbnb_listing_id', 'suburb']], on='airbnb_listing_id', how='inner'
    ).merge(revenue_df, on='airbnb_listing_id', how='inner')
    
    airbnb_roi = airbnb_roi[(airbnb_roi['number_of_bedrooms'] >= 1) & (airbnb_roi['number_of_bedrooms'] <= 6)]
    airbnb_grp = airbnb_roi.groupby(['suburb', 'number_of_bedrooms']).agg(
        mean_rev=('total_revenue', 'mean'), count_abnb=('airbnb_listing_id', 'count')
    ).reset_index().rename(columns={'number_of_bedrooms': 'bedrooms'})
    ...
    viva_valid = viva_valid[(viva_valid['bedrooms'] >= 1) & (viva_valid['bedrooms'] <= 6)]
    viva_grp = viva_valid.groupby(['suburb', 'bedrooms']).agg(
        mean_price=('sale_price', 'mean'), count_viva=('listing_id', 'count')
    ).reset_index()
    ...
    print(f"      - ROI Centro 1Q: {c1['roi_pct']:.2f}% | ROI Centro 2Q: {c2['roi_pct']:.2f}%")
    print(f"      - Custo Extra para o 2o Quarto: R$ {c2['mean_price'] - c1['mean_price']:,.0f}")
    print(f"      - Retorno Extra Anual Gerado: R$ {c2['mean_rev'] - c1['mean_rev']:,.0f}")
